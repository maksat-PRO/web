...
export function Platform() {
  ...
  const scoring = ["Founder Signal", "Market Demand", "Technical Depth", "Compliance Fit", "Global Readiness", "CVC Pull", "Exit Optionality"];
  const programs = [
  ...
  return (
    <div className="pt-24 sm:pt-28 lg:pt-32 pb-16 sm:pb-20 lg:pb-24 px-6 sm:px-8 lg:px-12">
      <div className="max-w-[1200px] mx-auto">
        ...
        <section className="mb-16 sm:mb-20 lg:mb-24 rounded-[26px] sm:rounded-[28px] bg-gradient-to-br from-[#1a1d26]/40 via-[#12141a]/50 to-[#1a1d26]/40 border border-white/[0.06] p-6 sm:p-10"><div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start"><div className="lg:col-span-4"><ScoreMeter score={84} label="Investment-ready" /></div><div className="lg:col-span-8"><span className="text-[12px] text-muted-foreground/50 font-light uppercase tracking-wider">Thesis + scoring</span><h2 className="text-[26px] sm:text-[34px] font-light tracking-tight text-foreground/90 mt-3 mb-4">Investment Readiness Score</h2><p className="text-[13px] sm:text-[15px] text-muted-foreground/70 font-light leading-relaxed mb-6">We look for overlooked technical alpha where founder quality, market demand, global readiness, compliance fit, and exit optionality can compound. This is an internal assessment framework, not a guarantee of investment.</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">{scoring.map((factor) => <div key={factor} className="rounded-[18px] bg-white/[0.025] border border-white/[0.05] p-5 text-[13px] text-muted-foreground/70 font-light">{factor}</div>)}</div></div></div></section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-16 sm:mb-20 lg:mb-24"><SignalCard eyebrow="Sourcing" title="Sourcing Corridors" description="We source from high-friction technical ecosystems and relocation hubs where strong founders often have product depth but need international packaging, compliance readiness, and trusted access to global capital."><div className="space-y-3">{corridors.map((item) => <div key={item} className="flex gap-3 text-[13px] sm:text-[14px] text-muted-foreground/70 font-light leading-relaxed"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/45" /><span>{item}</span></div>)}</div></SignalCard><SignalCard eyebrow="Operating discipline" title="Why it matters" description="Great venture work is not only access. It is discipline: the ability to see more founders, evaluate consistently, document decisions, manage conflicts, support portfolio companies, and report to LPs with evidence."><div className="space-y-3">{workflows.map((item) => <div key={item} className="flex gap-3 text-[13px] sm:text-[14px] text-muted-foreground/70 font-light leading-relaxed"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/45" /><span>{item}</span></div>)}</div></SignalCard></section>
      </div>
    </div>
  );
}
